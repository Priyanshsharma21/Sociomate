...
export const signup = async(req,res)=>{
    ...
    //validations    
    if(req.body===null) res.status(400).json({status:false,message:'empty data, fill the form!'});
    ...
    //signing up user 
    const userDetails = {name,mobile,email,password:hashedPass};
...
export const login = async (req,res)=>{
  try {
    ...
    if(!user)return res.status(400).json({status:false,message:'enter correct mobile or signup now!'});
    const checkPass = await bcrypt.compare(password,user.password);
    if(checkPass===false)return res.status(400).json({status:false,message:'password is incorrect!'});
    if(checkPass===true){
        const token = jwt.sign({userId:user._id.toString()},process.env.secretKey,{
            expiresIn:'3d'
        });
